...
comment_author.username AS "comment_author_username", comments.body
FROM users
INNER JOIN posts ON posts.user_id= users.id
INNER JOIN comments ON comments.user_id= posts.user_id
INNER JOIN users AS comment_author ON users.id = comments.user_id
WHERE (comments.body LIKE '%SSL%' OR comments.body LIKE '%firewall%') AND posts.content LIKE '%nemo%';

SELECT count(comments.body, posts.content)
FROM users
INNER JOIN posts ON posts.user_id= users.id
INNER JOIN comments ON comments.user_id= posts.user_id
INNER JOIN users AS comment_author ON users.id = comments.user_id
